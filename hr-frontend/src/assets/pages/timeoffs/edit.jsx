...
import {
  ...
  HiOutlineAnnotation, HiOutlinePaperClip, HiOutlineX, HiOutlineCheck,
  HiOutlineChevronDown, HiOutlineExclamation, HiOutlineCloudUpload,
  HiOutlineZoomIn,
} from "react-icons/hi";
...
const EditTimeOffPage = () => {
  const navigate = useNavigate();
  const { id }   = useParams();
  const { getTimeOffRequestById, updateTimeOffRequest } = useTimeOff();
  ...
  const [form, setForm] = useState({
    ...
    timeOffTypeId: "", startDate: "", endDate: "",
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const item  = reqRes?.data ?? reqRes;
        const list  = typesRes.data?.data ?? typesRes.data ?? [];
        const types = Array.isArray(list)
          ? list.filter((t) => !t.status || t.status === "ACTIVE")
          : [];
  ...
  const handleSubmit = async () => {
    if (!form.timeOffTypeId || !form.startDate || !form.endDate) {
      setError("Time Off Type, Start Date, dan End Date wajib diisi.");
      return;
    }
    ...
    try {
      // Upload attachment baru kalau ada file yang dipilih
      ...
      navigate("/time-off", {
        state: { toast: { show: true, message: "Request berhasil diperbarui.", type: "success" } },
      });
    } catch (err) {
  ...
  return (
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] flex flex-col overflow-hidden"
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <span className="text-sm font-semibold text-gray-700">
                {attachmentFile?.name || form.attachmentName || "Preview"}
              </span>
    ...
    <div className="w-full px-4 md:px-6 py-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <p className="text-xs text-gray-400 font-medium">
            ...
            <span className="text-gray-600">Edit Request</span>
          </p>
          <h1 className="text-2xl font-bold text-gray-800">Edit Time Off Request</h1>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {/* ══ Section 1: Employee (readonly) + Time Off Type ══ */}
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              <label className={labelCls}>
                <span className="flex items-center gap-1.5">
                  <HiOutlineDocumentText className="w-3.5 h-3.5" /> Employee
                </span>
              </label>
              <div className="px-3.5 py-2.5 text-sm bg-gray-100 border border-gray-200 rounded-xl text-gray-500 cursor-not-allowed">
                {form.employeeName || `Employee #${form.employeeId}` || "—"}
              </div>
              <p className="mt-1.5 text-xs text-gray-400">Tidak dapat diubah</p>
            </div>
            ...
            <div>
              <label className={labelCls}>
                <span className="flex items-center gap-1.5">
                  <HiOutlineDocumentText className="w-3.5 h-3.5" /> Time Off Type
            ...
            <div>
              ...
              {!form.attachmentUrl ? (
                ...
                <div className="relative group rounded-xl overflow-hidden border border-gray-200 h-[104px]">
                  <img src={form.attachmentUrl} alt="attachment"
                ...
                <div className="flex items-center gap-3 px-4 py-3 h-[104px] bg-indigo-50 border border-indigo-200 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-indigo-700 truncate">
                      {attachmentFile?.name || form.attachmentName || "File terlampir"}
                    </p>
                    {attachmentFile && (
        ...
        <div className="flex gap-3 pb-4">
          <button onClick={() => navigate("/time-off")} disabled={saving}
          ...
          <button onClick={handleSubmit} disabled={saving || exceedsMax}
            ...
            {saving ? (
              ...
              <><HiOutlineCheck className="w-4 h-4" /> Simpan Perubahan</>
            )}
          </button>
        </div>
      </div>
...
export default EditTimeOffPage;
