...
const CompanyInfoCard = ({ formData, handleChange, logoPreview, handleLogoChange }) => (
...
const AddCompany = () => {
  ...
  // Form Data - camelCase sesuai backend
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ✅ STEP 1: Upload logo dulu (jika ada)
      const logoUrl = logo ? await uploadFile(logo) : null;

      // ✅ STEP 2: Kirim JSON ke /companies
      const payload = {
      ...
      console.log('Submitting company with payload:', payload);
      
      // ✅ Ini akan mengirim Content-Type: application/json (default axios)
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full px-4 md:px-6 py-6 bg-gray-50 min-h-screen">
      ...
      {/* Card 1: Company Info (Logo + Company Name) */}
      <CompanyInfoCard 
        formData={formData}
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="border-b border-gray-200 bg-gray-50 px-6">
          <div className="flex space-x-6">
            <div className="py-3 px-1 text-sm font-medium border-b-2 border-indigo-600 text-indigo-600">
              General Information
            </div>
          </div>
        </div>
