...
const ProfileCard = ({ formData, handleChange, photoPreview, handlePhotoChange, employeeCode }) => (
...
const EditEmployee = () => {
  ...
  const loadEmployee = async () => {
    try {
      const response = await fetchEmployeeById(parseInt(id));
  ...
  // ===== HELPER =====
  const activeEmployees = employees?.filter(emp => emp.status === 'ACTIVE' && emp.id !== parseInt(id)) || [];

  // Set form data when employeeData is available
  useEffect(() => {
    if (employeeData) {
      setFormData({
        ...
        numberOfDependentChildren: employeeData.numberOfDependentChildren || '',
      ...
      setCitizenship({
        ...
        passportNumber: employeeData.passportNumber || '',
        familyCardNumber: employeeData.familyCardNumber || '',
        gender: employeeData.gender || '',
        dateOfBirth: employeeData.dateOfBirth || '',
        placeOfBirth: employeeData.placeOfBirth || ''
      });
  ...
  const handleChange = (e) => {
    ...
    // Update specific section states if needed
    if (name === 'privateAddress' || name === 'privateEmail' || name === 'privatePhone') {
      setPrivateContact(prev => ({ ...prev, [name.replace('private', '').toLowerCase()]: value }));
    } else if (name === 'bankName') {
      setBankAccounts(prev => prev.map(acc => ({ ...acc, bankName: value })));
    } else if (name === 'accountNumber') {
      setBankAccounts(prev => prev.map(acc => ({ ...acc, accountNumber: value })));
    } else if (name === 'npwpId') {
      setTaxInfo(prev => ({ ...prev, npwp: value }));
    } else if (name === 'homeToWorkDistance') {
      setTaxInfo(prev => ({ ...prev, workDistance: value }));
      ...
      setFamilyInfo(prev => ({ ...prev, maritalStatus: value }));
    } else if (name === 'numberOfDependentChildren') {
      setFamilyInfo(prev => ({ ...prev, numberOfChildren: value }));
    } else if (name === 'nationality') {
      setCitizenship(prev => ({ ...prev, nationality: value }));
    } else if (name === 'countryOfBirth') {
      setCitizenship(prev => ({ ...prev, countryOfBirth: value }));
    } else if (name === 'identificationNumber') {
      setCitizenship(prev => ({ ...prev, idNumber: value }));
    } else if (name === 'passportNumber') {
      setCitizenship(prev => ({ ...prev, passportNumber: value }));
    } else if (name === 'gender') {
      setCitizenship(prev => ({ ...prev, gender: value }));
    } else if (name === 'dateOfBirth') {
      setCitizenship(prev => ({ ...prev, dateOfBirth: value }));
    } else if (name === 'placeOfBirth') {
      setCitizenship(prev => ({ ...prev, placeOfBirth: value }));
    } else if (name === 'certificateLevel') {
      setEducation(prev => ({ ...prev, certificateLevel: value }));
      ...
      setSettings(prev => ({ ...prev, employeeType: value }));
    } else if (name === 'relatedUser') {
      setSettings(prev => ({ ...prev, relatedUserId: value }));
    } else if (name === 'monthlyCost') {
      setSettings(prev => ({ ...prev, monthlyCost: value }));
    } else if (name === 'attendanceBadgeId') {
      setSettings(prev => ({ ...prev, attendanceBadgeId: value }));
    }
  ...
  const handlePrivateContactChange = (e) => {
    ...
    setFormData(prev => ({ 
      ...prev, 
      [name === 'address' ? 'privateAddress' : name === 'email' ? 'privateEmail' : 'privatePhone']: value 
    }));
  };
  ...
  const handleCitizenshipChange = (e) => {
    ...
    if (fieldMap[name]) {
      setFormData(prev => ({ ...prev, [fieldMap[name]]: value }));
    }
  };
  ...
  const handleFamilyChange = (e) => {
    ...
    if (name === 'maritalStatus') {
      ...
      setFormData(prev => ({ ...prev, numberOfDependentChildren: value }));
    }
  };
  ...
  const handleTaxChange = (e) => {
    ...
    if (name === 'npwp') {
      setFormData(prev => ({ ...prev, npwpId: value }));
    } else if (name === 'workDistance') {
      setFormData(prev => ({ ...prev, homeToWorkDistance: value }));
    }
  };
  ...
  const handleSettingsChange = (e) => {
    ...
    if (type === 'checkbox') {
      ...
      } else if (name === 'relatedUserId') {
        setFormData(prev => ({ ...prev, relatedUser: value }));
      } else if (name === 'monthlyCost') {
        setFormData(prev => ({ ...prev, monthlyCost: value }));
      } else if (name === 'attendanceBadgeId') {
        setFormData(prev => ({ ...prev, attendanceBadgeId: value }));
      }
    }
  };
  ...
  const handleBankChange = (id, field, value) => {
    ...
    // Update formData with first bank account
    if (id === 1) {
      if (field === 'bankName') {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Upload photo jika ada perubahan
      const photoUrl = photo ? await uploadFile(photo) : formData.photo || null;
      ...
      const payload = {
        ...
        bankName: bankAccounts[0]?.bankName || null,
        accountNumber: bankAccounts[0]?.accountNumber || null,
        ...
        numberOfDependentChildren: familyInfo.numberOfChildren ? parseInt(familyInfo.numberOfChildren) : null,
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-gray-50 p-5 rounded-lg">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Status *
                    {errors.status && (
                      <span className="text-red-500 text-xs ml-2">{errors.status}</span>
                    )}
                  </label>
                  <select 
                    name="status" 
