...
const AddApproverModal = ({ approvers, employees, onAdd, onClose }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await onAdd({ employeeId: parseInt(form.employeeId), isRequired: form.isRequired, approvalOrder: nextOrder, minimumApproval: newMinimum });
      onClose();
    } catch (e) { setError(e?.message || "Failed to add approver"); }
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          ...
          <button onClick={onClose} className="p-1.5 hover:bg-gray-100 rounded-lg"><HiOutlineX className="w-4 h-4 text-gray-500" /></button>
        </div>
        <div className="px-5 py-4 space-y-4">
          ...
          <div>
            ...
            <select value={form.employeeId} onChange={(e) => setForm((p) => ({ ...p, employeeId: e.target.value }))}
              ...
              {active.map((e) => <option key={e.id} value={e.id}>{e.name}{e.employeeCode ? ` (${e.employeeCode})` : ""}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Required <span className="text-red-500">*</span></label>
            <div className="space-y-2">
              {[{ val: true, label: "Required" }, { val: false, label: "Optional" }].map((opt) => (
                <label key={String(opt.val)} className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-colors ${form.isRequired === opt.val ? "border-indigo-500 bg-indigo-50" : "border-gray-200 hover:border-gray-300"}`}>
                  <input type="radio" className="accent-indigo-600 w-4 h-4 flex-shrink-0" checked={form.isRequired === opt.val} onChange={() => setForm((p) => ({ ...p, isRequired: opt.val }))} />
                  <span className={`text-sm font-medium ${form.isRequired === opt.val ? "text-indigo-700" : "text-gray-700"}`}>{opt.label}</span>
                  {opt.val && <span className="ml-auto text-xs text-gray-400">Counts toward minimum approval</span>}
                </label>
              ))}
            </div>
          </div>
        ...
        <div className="px-5 pb-5 flex gap-2">
          ...
          <button onClick={handleSubmit} disabled={saving} className="flex-1 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm hover:bg-indigo-700 disabled:opacity-50 transition-colors">{saving ? "Saving…" : "Add Approver"}</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            ...
            <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><HiOutlineX className="w-5 h-5 text-gray-500" /></button>
          </div>
          <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-gray-700">Approvers <span className="ml-2 text-xs text-gray-400 font-normal">({sorted.length})</span></h3>
                <button onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 px-3 py-2 bg-indigo-600 text-white text-xs rounded-lg hover:bg-indigo-700 transition-colors">
                      ...
                      {sorted.map((ap) => (
                        <tr key={ap.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-right">
                            <button onClick={() => handleDelete(ap.id)} disabled={deleting === ap.id} className="p-1.5 hover:bg-red-50 rounded-lg text-red-400 transition-colors disabled:opacity-50">
            ...
            <div className={`rounded-xl border px-5 py-4 flex items-center gap-4 ${minimumApproval > 0 ? "bg-indigo-50 border-indigo-200" : "bg-gray-50 border-gray-200"}`}>
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${minimumApproval > 0 ? "bg-indigo-600" : "bg-gray-300"}`}>
                <HiOutlineShieldCheck className="w-5 h-5 text-white" />
              </div>
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-semibold ${minimumApproval > 0 ? "text-indigo-800" : "text-gray-600"}`}>Minimum Approval</p>
                <p className="text-xs text-gray-500 mt-0.5">Auto-calculated from approvers with <span className="font-medium text-red-500">Required</span> status</p>
              </div>
              <div className={`text-3xl font-bold tabular-nums ${minimumApproval > 0 ? "text-indigo-600" : "text-gray-400"}`}>{minimumApproval}</div>
            </div>
            {minimumApproval === 0 && sorted.length > 0 && (
              <div className="flex items-start gap-2 px-3 py-2.5 bg-yellow-50 border border-yellow-200 rounded-xl">
                ...
                <p className="text-xs text-yellow-700">No approvers are marked as <span className="font-semibold">Required</span>. Minimum approval is 0 — all approvers are optional.</p>
              </div>
            )}
          </div>
        </div>
      </div>
      {showAdd && <AddApproverModal approvers={approvers} employees={employees} onAdd={onAddApprover} onClose={() => setShowAdd(false)} />}
    </>
  );
};
    ...
    >
      <div className="relative w-fit mb-4">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${
          tab.available ? hasPending ? "bg-red-50 group-hover:bg-red-500" : "bg-indigo-100 group-hover:bg-indigo-600" : "bg-gray-100"
        }`}>
          ...
          {menuOpen && (
            <div className="absolute right-0 top-8 z-30 bg-white border border-gray-200 rounded-xl shadow-lg py-1 w-48">
              <button onClick={() => { setMenuOpen(false); onOpenSettings(); }}
                ...
                <HiOutlineShieldCheck className="w-4 h-4 text-indigo-500" /> Approval Settings
              </button>
            </div>
          )}
        </div>
      ...
      {tab.available && (
        <div className={`absolute bottom-5 right-5 transition-colors ${hasPending ? "text-red-300 group-hover:text-red-500" : "text-gray-300 group-hover:text-indigo-500"}`}>
