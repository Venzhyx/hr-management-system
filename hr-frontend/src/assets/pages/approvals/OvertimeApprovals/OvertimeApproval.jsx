...
import OvertimeDetailModal from "./OvertimeApprovalModal";
...
// ── Helper: hitung status berdasarkan approvals ───────────────────────────────
...
const ApprovalOvertimePage = () => {
  const { overtimes, loading, fetchOvertimes } = useOvertime({ role: "admin" });
  ...
  useEffect(() => {
    if (selected && overtimes) {
      const updated = overtimes.find((o) => o.id === selected.id);
  ...
  const stats = (overtimes || []).reduce(
    (acc, o) => {
      ...
      acc.total++;
      if (displayStatus === "SUBMITTED") acc.submitted++;
      if (displayStatus === "PENDING") acc.pending++;
      if (displayStatus === "APPROVED") acc.approved++;
      if (displayStatus === "REJECTED") acc.rejected++;
      return acc;
    },
  ...
  const filtered = sorted.filter((o) => {
    const displayStatus = getDisplayStatus(o);
    if (!activeFilters.includes(displayStatus)) return false;
    if (search) {
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {["Employee", "Tanggal", "Tipe", "Mulai", "Selesai", "Total Jam", "Status", "Aksi"].map((h) => (
              ...
              {filtered.map((o) => {
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-full overflow-hidden flex-shrink-0 ring-1 ring-gray-200">
                          {emp?.photo ? (
                            <img src={emp.photo} alt={o.employeeName} className="w-full h-full object-cover" />
                          ) : (
                    ...
                    <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{fmtDateShort(o.date)}</td>
                    <td className="px-4 py-3">
